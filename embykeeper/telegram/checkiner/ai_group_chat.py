...
class AiGroupChatCheckin(BotCheckin):
    ...
    async def get_ai_poetry(self):
        """调用hohai.eu.org AI接口获取诗句，失败时自动降级到本地诗句库"""
        # 使用内置的 token 和模型
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                data = {
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": "请生成一句优美的中国古诗句，要求：1. 字数在5-10字之间；2. 不要包含标点符号；3. 语言优美典雅；4. 可以是唐诗、宋词或元曲风格"
                        }
                    ]
                }
                
                self.log.info(f"调用AI接口，使用模型: {model}")
                ...
                if response.status_code == 200:
                    ...
                    self.log.info(f"成功从AI接口获取诗句: {cleaned_content}")
            ...
            self.log.warning(f"AI接口调用异常，自动切换到本地诗句: {e}")
